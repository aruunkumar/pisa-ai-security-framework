...
class AutomatedReasoningValidator:
    ...
    def _parse_extracted_intent(self, extracted_intent: str) -> Dict:
        ...
        # Fallback: If no amount found in PARAMETERS, check the entire extracted_intent
        if 'amount' not in parsed['parameters']:
            amount_match = re.search(r'\$?([\d,]+(?:\.\d{2})?)', extracted_intent)
            if amount_match:
                parsed['parameters']['amount'] = float(amount_match.group(1).replace(',', ''))
            ...
            # Extract loan type from PARAMETERS
        ...
        # Fallback: Check entire extracted_intent for loan type if not found
        if 'loan_type' not in parsed['parameters']:
            full_text = extracted_intent.lower()
            if 'mortgage' in full_text:
                parsed['parameters']['loan_type'] = 'mortgage'
            elif 'auto' in full_text or 'vehicle' in full_text:
                parsed['parameters']['loan_type'] = 'auto_loan'
            elif 'personal' in full_text:
                parsed['parameters']['loan_type'] = 'personal_loan'
    ...
    def _validate_wire_transfer_policy(
        ...
        logger.info(f"Wire transfer validation: amount=${amount:,.2f}, agent_limit=${agent_limit:,.2f}, agent={agent_level}")
    ...
    def _validate_loan_policy(
        ...
        # Fallback: Check original prompt for loan type if not found
        if loan_type == 'personal_loan':
            prompt_lower = original_prompt.lower()
            if 'mortgage' in prompt_lower:
                loan_type = 'mortgage'
            elif 'auto' in prompt_lower or 'vehicle' in prompt_lower:
                loan_type = 'auto_loan'
        
        agent_limits = self.rules['loan_approval']['agent_limits'].get(agent_level, {})
        agent_limit = agent_limits.get(loan_type, 0)
        
        logger.info(f"Loan validation: amount=${amount:,.2f}, loan_type={loan_type}, agent_limit=${agent_limit:,.2f}, agent={agent_level}")
